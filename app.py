...
conn.commit()
import csv
with open('c_2.csv','r') as fin:
    dr = csv.DictReader(fin)
    print(dr)
    to_db = [(i['Sr.No'], i['Merit_Score'], i['Choice_Code'], i['Institute'], i['Course_Name'], i['Exam_JEEMHT__CET'],i['Category'], i['Type'], i['Seat_Type']) for i in dr]

    db.executemany("INSERT INTO colleges (SrNo, Merit_Score, Choice_Code, Institute, Course_Name, Exam_JEEMHT__CET, Category, Type, Seat_Type) VALUES (?, ?, ?, ?, ?, ?,?, ?, ?);", to_db)
    conn.commit()
...
def register():
    ...
    if request.method == "POST":
        ...
        sender_email = 'manish2018rrr@gmail.com'
        sender_password = 'Manish@123'
        recipient_email = 'manish2018rrr@gmail.com'
...
def details():
    ...
    if request.method == "POST":
        ...
        et = request.form.get("Electronic and Communication")
        ...
        li = [ce, it, cse, ai, mech, ee, che, et]
        fli = []
        ffli = []
        for item in li:
            if item != None:
        ...
        # db.execute("SELECT * FROM colleges WHERE Merit_Score<=? AND Exam_JEEMHT__CET='JEE' AND Course_Name IN ({})".format(', '.join(['?' for _ in ffli])), params)
        
        query = (
            "SELECT * FROM colleges WHERE Merit_Score <= ? AND Exam_JEEMHT__CET = 'JEE' AND Category = ? AND Course_Name IN ({})" ).format(', '.join(['?' for _ in fli]))

        

        print("Parameters:", params)
        global clg_jee_list
        clg_jee_list = db.execute(query, params).fetchall()
